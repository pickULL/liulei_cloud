package com.liulei.cloud.utils;
...
import static com.liulei.cloud.enums.SensitiveType.ID_CARD;
...
//----------------------------------------------------------------------------------------------  
     ...
     * 注解处理器
     */
    public static void dealSensitiveInfo(Object obj) throws IllegalAccessException {
        ...
        for(Field field : fields){
            if(field.isAnnotationPresent(SensitiveInfo.class)) {
                ...
                switch (annotation.type()) {
                    case CHINESE_NAME: {
                        field.set(obj,SensitiveInfoUtils.chineseName(field.get(obj).toString()));
                        break;
                    }
                    case ID_CARD: {
                        field.set(obj,SensitiveInfoUtils.idCardNum(field.get(obj).toString()));
                        break;
                    }
                    case FIXED_PHONE: {
                        field.set(obj,SensitiveInfoUtils.fixedPhone(field.get(obj).toString()));
                        break;
                    }
                    case MOBILE_PHONE: {
                        field.set(obj,SensitiveInfoUtils.mobilePhone(field.get(obj).toString()));
                        break;
                    }
                    case ADDRESS: {
                        field.set(obj,SensitiveInfoUtils.chineseName(field.get(obj).toString()));
                        SensitiveInfoUtils.address(field.get(obj).toString(), 4);
                        break;
                    }
                    case EMAIL: {
                        field.set(obj,SensitiveInfoUtils.email(field.get(obj).toString()));
                        break;
                    }
                    case BANK_CARD: {
                        field.set(obj,SensitiveInfoUtils.bankCard(field.get(obj).toString()));
                        break;
                    }
                    case CNAPS_CODE: {
                        field.set(obj,SensitiveInfoUtils.cnapsCode(field.get(obj).toString()));
                        break;
                    }
                }
            }
